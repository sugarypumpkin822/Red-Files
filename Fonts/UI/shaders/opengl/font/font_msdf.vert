...
// Multi-channel Signed Distance Field (MSDF) Font Vertex Shader
// Enhanced vertex processing for MSDF font rendering
...
uniform float uPixelRange;              // MSDF pixel range
uniform vec2 uTextureSize;              // Texture size for MSDF
uniform float uScreenScale;             // Screen scale for MSDF
...
vec3 applySlideIn(vec3 position, float time)
{
    ...
    vec3 slideOffset = uSlideInDirection * uSlideInDistance * (1.0 - slideProgress);
...
// Calculate pixel range for MSDF
float calculatePixelRange(vec2 screenSize, vec2 textureSize, float screenScale)
{
    if (uAutoPixelRange)
    {
        // Auto-calculate based on screen and texture size
        return uPixelRange * length(screenSize / textureSize) * screenScale;
    }
    else
    {
        return uPixelRange;
...
void main()
{
    ...
    // Calculate MSDF-specific values
    vTexelSize = 1.0 / uTextureSize;
    vPixelRange = calculatePixelRange(uScreenSize, uTextureSize, uScreenScale);
